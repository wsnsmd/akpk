  ...
  const destroy = (id) => {
    delData().then((result) => {
      if (result.isConfirmed) {
        Inertia.delete(route('ubar.destroy', id));
      }
    })
  }
    ...
    <div>
      <div className="border-b border-gray-200 px-4 py-4 sm:flex sm:items-center sm:justify-between sm:px-6 lg:px-8">
        <div className="flex-1 min-w-0 py-2">
          <h1 className="text-lg font-medium leading-6 text-gray-900 sm:truncate">Usulan Pelatihan Baru</h1>
        </div>
      </div>
      {inputdata &&
...
Index.layout = page => <Layout title="Usulan Pelatihan Baru" children={page} />;
