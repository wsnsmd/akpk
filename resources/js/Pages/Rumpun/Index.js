  ...
  function destroy(id) {
    delData().then((result) => {
      if (result.isConfirmed) {
        Inertia.delete(route('rumpun.destroy', id));
      }
    })
  }
        ...
        <table className="min-w-full whitespace-nowrap">
          <thead className="bg-gray-50">
            <tr className="border-t border-gray-200">
              ...
              <th className="px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kode</th>
          ...
          <tbody>
            {data.map(({ id, kode, nama, ket, deleted_at, i }) => {
              return (
                <tr
                  key={i}
